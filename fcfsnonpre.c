...
void main()
{
    int n,pid[25],bt[25],wt[25],i,tat[25],at[25];
    ...
    for (  i = 1; i < n; i++)
    {
         wt[i]=wt[i-1]+bt[i-1]-at[i]+at[i-1];                 //calculating the waiting time
    }
    float twt=0.0;
    float ttat=0.0;
    printf("\nPROCESS ID     WAITING TIME     BURST TIME     ARRIVAL TIME     TURNAROUND TIME\n");
    for (  i = 0; i < n; i++)
    {
         printf("%d\t\t",pid[i]);
         printf("%d\t\t",wt[i]);
